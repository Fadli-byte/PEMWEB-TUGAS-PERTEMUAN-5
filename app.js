...
app.get('/', (req, res) => {
    connection.query('SELECT * FROM users', (err, result) => { // Query SQL untuk mengambil semua data dari tabel 'users'
        ...
        res.render('index', { users: result }); // Render file 'index.ejs' dengan data user yang diambil dari database
    });
});

// Route untuk Create (menambahkan user baru)
app.post('/add', (req, res) => {
    const { name, email, phone } = req.body; // Mengambil data dari form input user
    const query = 'INSERT INTO users (name, email, phone) VALUES (?, ?, ?)'; // Query SQL untuk menambahkan user baru ke database
    connection.query(query, [name, email, phone], (err, result) => { // Eksekusi query dengan data user
...
app.post('/update/:id', (req, res) => {
    const { name, email, phone } = req.body; // Mengambil data yang telah diubah dari form
    const query = 'UPDATE users SET name = ?, email = ?, phone = ? WHERE id = ?'; // Query SQL untuk memperbarui data user di database
...
app.listen(3000, () => {
    console.log('Server berjalan di port 3000. Akses melalui http://localhost:3000'); // Menjalankan server di port 3000
});
